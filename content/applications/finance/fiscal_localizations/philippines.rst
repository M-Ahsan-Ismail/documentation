...
:abbr:`Alpha (Alphalist Data Entry and Validation)` module.
...
SLS and SLP reports can be exported in `.DAT` format, which is compatible with the :abbr:`BIR ReLiEf
(Reconciliation of Listings for Enforcement)` module.

#. Make sure to filter by the appropriate month.
#. On the report page, click the :icon:`fa-cog` :guilabel:`(gear)` icon and select :guilabel:`Export
   SLSP`.
#. In the :guilabel:`DAT Export Options` window, choose the relevant :guilabel:`Attachment For` and
