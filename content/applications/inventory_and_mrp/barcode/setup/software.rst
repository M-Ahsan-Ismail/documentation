...
The barcode scanning features can save a lot of time usually lost switching between the keyboard,
the mouse, and the scanner. Properly attributing barcodes to products, product packagings, picking
locations, and more ensures more efficient workflows by controlling the software almost exclusively
with the barcode scanner.

Configuration
=============

To use this feature, navigate to :menuselection:`Inventory --> Configuration --> Settings`. In the
*Barcode* section, select :guilabel:`Barcode Scanner`. Be sure to click :guilabel:`Save`.
...
Odoo supports GTIN barcodes to identify their products. However, since Odoo supports any numeric
string as a barcode, it is also possible to define a custom barcode for internal use.
