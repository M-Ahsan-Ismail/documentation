...
The signature request is then marked as :guilabel:`Cancelled` and the refusal is recorded in
the :ref:`signature request's activity logs and chatter <sign/request-signatures/manage>`. Any other
signers are notified and any remaining signers can no longer sign the document.
